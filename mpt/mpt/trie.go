...
func (t *Trie) Get(key []byte) ([]byte, error) {
	...
	if err != nil {
		...
		return nil, errors.New(fmt.Sprintf("[A.Trie] key not found: %s", hex.EncodeToString(key)))
	}
}

func (t *Trie) get(node Node, key []byte, prefixLen int) (Node, Node, error) {
	if node == nil {
		return nil, node, errors.New(fmt.Sprintf("[B.Trie] key not found: %s", hex.EncodeToString(key)))
	}
	switch n := node.(type) {
	case *FullNode:
		if prefixLen > len(key) {
			return nil, node, errors.New(fmt.Sprintf("[C.Trie] key not found: %s", hex.EncodeToString(key)))
		}
		if prefixLen == len(key) {
	...
	case *ShortNode:
		if len(key)-prefixLen < len(n.Key) || !bytes.Equal(n.Key, key[prefixLen:prefixLen+len(n.Key)]) {
			return nil, node, errors.New(fmt.Sprintf("[D.Trie] key not found: %s", hex.EncodeToString(key)))
		}
